...
-- Thời gian đã tạo: Th3 28, 2025 lúc 03:45 PM
...
SELECT * FROM users WHERE user_name LIKE '%a%' AND LENGTH(user_name) BETWEEN 5 AND 9 AND user_email LIKE '%@gmail.com' AND LEFT(user_email, LOCATE('@', user_email) - 1) LIKE '%i%';
...
SELECT * FROM users WHERE (user_name LIKE '%a%' AND LENGTH(user_name) BETWEEN 5 AND 9) OR (user_name LIKE '%i%' AND LENGTH(user_name) < 9) OR (user_email LIKE '%@gmail.com' AND LEFT(user_email, LOCATE('@', user_email) - 1) LIKE '%i%');